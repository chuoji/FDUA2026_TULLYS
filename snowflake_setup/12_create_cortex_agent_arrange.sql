...
CREATE OR REPLACE AGENT FINANCIAL_DATA_AGENT_ARRANGE
  ...
  PROFILE = '{
    "display_name": "財務データアナリスト（チャンク＆外部データ取り込み版）",
    "avatar": "🏢",
    "color": "#1E3A8A"
  }'
  FROM SPECIFICATION
...
instructions:
  system: |
    ...
    5. PrefResidentialBasicInfo
      - 総務省が出している都道府県データ 基礎指標 
      - 都道府県ごとの「住宅ストック・着工・居住水準・設備・都市機能」などの基礎指標
      - 以下観点で整理
         * 地域: 都道府県などの地域単位
         * 住宅ストック/構造/建て方: 総住宅数、持ち家/借家、木造/非木造、共同住宅/戸建など
         * 築年・更新: 1970年以前/1970年代/1980年代…などの築年帯別住宅数、滅失/除却など
         * 着工関連: 着工居住用建築物数、着工新設住宅戸数、持家/貸家/分譲など
         * 居住水準: 1住宅当たり居住室数、延べ面積、畳数、最低/誘導居住面積水準など
         * 設備・性能: 耐震改修、太陽光/太陽熱、複層ガラス等の住宅性能・設備
         * 生活インフラ/都市機能: 水道、衛生、ごみ、道路、公園、通信などの関連指標

    6. PrefResidentialSocialLifeInfo
      - 総務省が出している都道府県データ 社会生活統計指標 
      - 都道府県ごとの「住宅比率・居住水準・生活利便・都市計画・インフラ指標」を横断して参照
      - 以下観点で整理
         * 地域: 都道府県などの地域単位
         * 住宅市場（比率）: 着工新設住宅比率、持ち家比率、借家比率、民営借家比率、戸建/共同住宅比率、空き家比率など
         * 居住水準（質）: 居住室数、畳数、延べ面積、最低居住面積水準以上世帯割合など
         * コスト: 民営賃貸住宅家賃、着工工事費単価など
         * 道路・都市計画・公園: 道路延長/舗装率、用途地域比率、公園面積/数など

    7. ConstructionReferenceInitiatives
      - 実在する建設企業の有価証券報告書から切り出した「取り組み内容」を参照
      - 有価証券報告書内の以下の項目についての「取り組み内容」
        * 経営方針、経営環境及び対処すべき課題等
        * サステナビリティに関する考え方及び取組
        * 事業等のリスク
...
tools:
  ...
  - tool_spec:
      type: "cortex_analyst_text_to_sql"
      name: "PrefResidentialBasicInfo"
      description: |
        都道府県ごとの「住宅ストック・着工・居住水準・設備・都市機能」などの基礎指標を参照・分析するツール。
        仮想建設企業の地域提案（需要/供給・更新需要・居住品質・防災/省エネ投資余地）の根拠データとして利用します。
        以下のデータを含みます:
        - 調査年: 指標の時点（年/年度）
        - 地域: 都道府県などの地域単位
        - 居住: 居住区分（居住形態など）
  ...
  - tool_spec:
      type: "cortex_analyst_text_to_sql"
      name: "PrefResidentialSocialLifeInfo"
      description: |
        都道府県ごとの「住宅比率・居住水準・生活利便・都市計画・インフラ指標」を横断して参照・分析するツール。
        仮想建設企業の地域提案（賃貸市場、生活利便、都市構造、インフラ更新・維持管理）の根拠データとして利用します。
        以下のデータを含みます:
        - 調査年: 指標の時点（年/年度）
        - 地域: 都道府県などの地域単位
        - 居住: 居住区分（居住形態など）
        ...
        - 居住水準（質）: 居住室数、畳数、延べ面積、最低居住面積水準以上世帯割合など
        - コスト: 民営賃貸住宅家賃、着工工事費単価など
        - エネルギー: ガソリン販売量、発電電力量、電力需要量など
        - 水道・衛生: 上水道給水人口比率、し尿処理人口比率など
        - 廃棄物: リサイクル率、埋立率、最終処分場残余容量など
        - 生活利便: 理容・美容、クリーニング、公衆浴場、給油所、郵便局などの施設密度
        - 通信: 固定電話/住宅用電話、公衆電話、携帯契約などの通信関連指標
        - 道路・都市計画・公園: 道路延長/舗装率、用途地域比率、公園面積/数など

  - tool_spec:
      type: "cortex_analyst_text_to_sql"
      name: "ConstructionReferenceInitiatives"
      description: |
        実在する建設企業の有価証券報告書から切り出した「取り組み内容」を参照・分析するツール。
        仮想建設企業の企業戦略書（提案書）作成における参考資料として利用します。
        以下のデータを含みます:
        - 企業: 参考情報の対象となる実在企業（社名）
        - 項目: 有価証券報告書内の項目（経営方針、経営環境及び対処すべき課題等 / サステナビリティに関する考え方及び取組 / 事業等のリスク / 経営者による財政状態、経営成績及びキャッシュ・フローの状況の分析 / 研究開発活動）
        - 取り組み内容: 各項目の取り組み内容（重要: Cortex Analyst検索対象）

        このツールを使用して、各社の経営方針・課題認識、サステナビリティ施策、
        リスク要因と対応、財政状態・経営成績の分析（MD&A）、研究開発の方向性など、
        戦略書作成に必要な定性情報を抽出できます。
...
tool_resources:
  ...
  PrefResidentialBasicInfo:
    semantic_view: "FDUA_COMPETITION.PUBLIC.PREF_RES_BASIC_PROPOSAL_SEMANTIC"
    execution_environment:
      type: warehouse
      warehouse: COMPUTE_WH
      
  PrefResidentialSocialLifeInfo:
    semantic_view: "FDUA_COMPETITION.PUBLIC.PREF_RES_SOCIAL_LIFE_PROPOSAL_SEMANTIC"
    execution_environment:
      type: warehouse
      warehouse: COMPUTE_WH

  ConstructionReferenceInitiatives:
    semantic_view: "FDUA_COMPETITION.PUBLIC.CONSTRUCTION_REFERENCE_INITIATIVES"
    execution_environment:
      type: warehouse
      warehouse: COMPUTE_WH
